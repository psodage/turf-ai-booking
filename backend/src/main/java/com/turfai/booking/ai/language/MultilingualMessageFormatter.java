...
public class MultilingualMessageFormatter {
    ...
    public MenuConfig getMenuConfig(String lang) {
        ...
        String header = switch (language) {
            case "HI" -> "👋 Green Pitch Kolhapur में आपका स्वागत है";
            case "MR" -> "👋 Green Pitch Kolhapur मध्ये आपले स्वागत आहे";
            default -> "👋 Welcome to Green Pitch Kolhapur";
        };

        String body = switch (language) {
            case "HI" -> "कृपया नीचे दिए गए विकल्पों में से चुनें या अपना प्रश्न लिखें।";
            case "MR" -> "कृपया खालील पर्यायांपैकी निवडा किंवा तुमचा प्रश्न टाईप करा.";
            case "HINGLISH" -> "Kripya neeche diye gaye options mein se select karein ya apna query type karein.";
            case "MINGLISH" -> "Krupaya khalil options pakin select kara kiwa tumcha query type kara.";
            default -> "Please select an option below or type your query.";
        };
        ...
        String secTitle = switch (language) {
            case "HI" -> "मुख्य विकल्प";
            case "MR" -> "मुख्य पर्याय";
            default -> "Menu Options";
        };

        List<OutboundRow> rows = switch (language) {
            case "HI" -> List.of(
        ...
        return new MenuConfig(header, body, buttonText, List.of(OutboundSection.builder().title(secTitle).rows(rows).build()));
    }
